...
class ThumbnailTask():
    ...
    def make_video_thumbnail(self, video_path, tn_video_image_path, stime=0, etime=0):
        ...
        fix_video_path = ''
        if task.check_file_exist(video_path):
    ...
    def make_thumbnail(self, param_dict):
        ...
        crop_base_poster_path = "%s%s_poster%s%s" % (param_dict['output'], name_lb, name_time, self.poster_ext)
        ...
        poster_path = "%s%s_tposter%s_0x0_0.png" % (param_dict['output'], name_lb, name_time)

        if param_dict['force'] == 1 or (not os.path.exists(task.utf8_to_file_charset(poster_path))):
            if mediaformat == task.URL_TYPE_VIDEO:
            ...
            poster_dir, file_name = os.path.split(param_dict['output'])
        ...
        if not os.path.exists(task.utf8_to_file_charset(crop_base_poster_path)):
            crop_base_poster_path = "%s%s_poster%s.png" % (param_dict['output'], name_lb, name_time)

        # size
        if param_dict['size'] in ['0x0', '0']:
            tn_size_image_path = crop_base_poster_path
        else:
            tn_size_image_path = "%s%s_tposter%s_%s_0%s" % (param_dict['output'], name_lb, name_time, param_dict['size'], self.poster_ext)
